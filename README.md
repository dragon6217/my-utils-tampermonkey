...
* **사용 방법:**
    1.  첫 번째 탭(예: 원문 가사)에서 Tampermonkey 아이콘을 클릭하고 **`Add to Sync List`**를 선택합니다.
    2.  메뉴가 갱신되며 `Slot 1`에 해당 탭의 제목이 등록된 것을 확인합니다. (아래 첫 번째 이미지 참고)
    3.  두 번째 탭(예: 번역본)에서도 **`Add to Sync List`**를 선택하면 `Slot 2`에 등록됩니다.
    4.  두 슬롯이 모두 채워지면 스크롤 동기화가 즉시 활성화됩니다. (아래 두 번째 이미지 참고)
    5.  동기화를 중지하려면 메뉴의 **`Remove from Sync List`** (해당 탭만 제거) 또는 **`Clear Sync List`** (모든 탭 제거)를 선택합니다.
